  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" id="trust-strip">
      <motion.div
        ...
        /* gap-px over a tinted container draws clean hairlines between cells at
           every breakpoint. `divide-*` only worked in the 1- and 4-column cases
           and left the 2-column layout with no separators at all. */
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-slate-200 border border-slate-200 rounded-2xl shadow-sm overflow-hidden"
      >
        {PILLARS.map((p) => (
          <motion.div
            key={p.title}
            variants={item}
            className="flex items-start gap-3.5 p-5 lg:p-6 bg-white hover:bg-slate-50/70 transition-colors"
          >
            <p.icon size={22} className={`${p.tone} flex-shrink-0 mt-0.5`} strokeWidth={1.9} />
            <div className="min-w-0">
              <div className="text-[14px] font-medium text-slate-900">{p.title}</div>
