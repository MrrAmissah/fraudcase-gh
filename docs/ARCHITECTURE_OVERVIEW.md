...
### Gemini API
- Called from `src/lib/gemini/analyzeFraudCase.ts` via `@google/genai`, model `gemini-3.1-flash-lite`, with `responseMimeType: application/json` and a `responseSchema` (`src/lib/gemini/fraudCaseSchema.ts`) so output is typed JSON matching `FraudAnalysis`.
